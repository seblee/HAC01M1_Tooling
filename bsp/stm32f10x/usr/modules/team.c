...
static uint16_t calc_team_config_check(void)
{
    ...
    for(i = 0; i < TEAM_CONF_MAX; i++)
    {
        if(i == 0)
        {
            team_local_inst.team_config[i] = team_local_inst.team_config[i] & ABNORMAL_VALUE;
        }
...
//		extern local_reg_st l_sys;	
    ...
    time(&now);
//		rt_kprintf("set is %d now is %d u32L_Time= %d\n", team_local_inst.rotate_time_t,now,l_sys.u32L_Time);
    now += 28800;
...
//		rt_kprintf("%d\n", team_local_inst.rotate_timeout);
		...
		switch(period)
		{
				...
				{
//						rt_kprintf("ti->tm_hour= %d ti->tm_min= %d ti->tm_sec= %d hour= %d min= %d\n", ti->tm_hour, ti->tm_min,  ti->tm_sec, hour, min);
