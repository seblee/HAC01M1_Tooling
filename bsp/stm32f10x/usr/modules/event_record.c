...
//		//Alair,20170227
//		if(((get_alarm_bitmap(ACL_WATER_OVERFLOW) == 1)&&(g_sys.config.alarm[ACL_WATER_OVERFLOW].alarm_param == 0))
//			||((get_alarm_bitmap(ACL_WATERPAN) == 1)&&(g_sys.config.alarm[ACL_WATERPAN].alarm_param == 0)))
		{
				req |= 1;
				return(req);
		}
...
////		//Alair,20170115
	...
	//Alair,20170222
	if((g_sys.status.ControlPassword.Run_State&WORK_PASS_ALL))//开机管控提示告警
	{
			major_cnt++;		
	}
...
static void calc_hum_temp(void)
{
	...
	for(i=0;i<60;i++)
	{
		if(hum_temp_log_inst.tem_hum_buff[i].hum == ABNORMAL_VALUE)
		{
				hum_flag = 1;;
		}
		else
		...
		if(hum_temp_log_inst.tem_hum_buff[i].temp == ABNORMAL_VALUE)
		{
				temp_flag= 1;
		}
		else
