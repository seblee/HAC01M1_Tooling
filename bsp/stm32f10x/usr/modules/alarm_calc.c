...
//		acl09,//		
		acl08,//		送风温湿度传感器故障,Alair,20161125
		acl09,//		回风温湿度传感器故障
...
//		61,     3,    4,  MIOOR_ALARM_LEVEL,     DEV_TYPE_OTHER,//ACL_FILTER_CLOG
		61,     3,    4,  MAJOR_ALARM_LEVEL,     DEV_TYPE_OTHER,//ACL_FILTER_CLOG//过滤网堵
...
//	(get_alarm_bitmap(ACL_SHORT_TERM1))||(get_alarm_bitmap(ACL_INV_FAULT))||
  ...
  ((get_alarm_bitmap(ACL_EX_FAN_CURRENT)))||	//外风机故障
	(get_alarm_bitmap(ACL_INV_HIPRESS)))//高压
	{
		compress1_alarm=1;
	}
	else
...
//			return(ALARM_ACL_HOLD);
//	}	
//	if(g_sys.config.fan.num>=2)//DI复用
//	{
...
//		}
////		//Alair,20170224
////		if((g_sys.config.general.cool_type == COOL_TYPE_COLUMN_WIND)&&(g_sys.config.compressor.type == COMP_QABP))//列间变频，用作电子膨胀阀故障
////		{
////				return(ALARM_ACL_CLEARED);		
////		}
//		if(sys_get_pwr_sts() == 0)
...
//	}
//		//Alair,20180310
//	if(g_sys.config.compressor.type == COMP_QABP)//变频
//	{
...
//			{			
//					if(g_sys.status.inv_compress_alarm.Inv_hi_Temperature[GET_AVG]>=g_sys.config.alarm[ACL_EXTMP1].alarm_param)//  排气温度大于阈值
//					{
//							return(1);				
//					}
//			}
...
//	if(g_sys.config.compressor.type == COMP_QABP)//变频
//	{
//			if(g_sys.status.inv_compress_alarm.avg_hi_press[GET_AVG]>g_sys.config.alarm[ACL_INV_HIPRESS].alarm_param)//高压压力大于阈值
//			{
//					return(1);				
//			}
