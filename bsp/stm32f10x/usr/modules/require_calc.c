...
//		}
		...
		if(current_temp >= ABNORMAL_VALUE)
		{
				temp_req = 0;
		}
...
//		}
		...
		//当前温度
		g_sys.status.sys_tem_hum.Current_air_temp=current_temp;
		g_sys.status.sys_tem_hum.Target_air_temp=set_temp;		
		if(current_temp >= ABNORMAL_VALUE)
		{
				//
				g_sys.status.sys_tem_hum.Current_air_temp=set_temp;
				return 0;
		}		
...
static int16_t local_hum_req_calc(void)
{
		...
		//当前湿度
		g_sys.status.sys_tem_hum.Current_air_hum=current_hum;
		g_sys.status.sys_tem_hum.Target_air_hum=set_hum;
		if(current_hum >= ABNORMAL_VALUE)
		{
				g_sys.status.sys_tem_hum.Current_air_hum=set_hum;
				return 0;
		}	
		else
		{
				...
				default:
						{
							...
							switch(g_sys.config.fan.mode)
							{
								case FAN_MODE_TEMP_DIFF:
										real_temp = abs(g_sys.status.sys_tem_hum.return_air_temp - g_sys.status.sys_tem_hum.supply_air_temp);
										if((g_sys.status.sys_tem_hum.return_air_temp  == ABNORMAL_VALUE)||(g_sys.status.sys_tem_hum.supply_air_temp == ABNORMAL_VALUE))
										{
												real_temp = ABNORMAL_VALUE;	
										}
									break;
								case FAN_MODE_TEMP_MAX_DIFF:
										real_temp = abs(g_sys.status.sys_tem_hum.return_air_max_temp - g_sys.status.sys_tem_hum.supply_air_min_temp);	
										if((g_sys.status.sys_tem_hum.return_air_max_temp  == ABNORMAL_VALUE)||(g_sys.status.sys_tem_hum.supply_air_min_temp == ABNORMAL_VALUE))
										{
												real_temp = ABNORMAL_VALUE;	
										}
									break;
					...
					// valid temp
							if(real_temp == ABNORMAL_VALUE)
							{
								real_temp = target_temp;
							}
							g_sys.status.sys_tem_hum.Current_fan_temp=real_temp;
					//int16_t p_algorithm(int16_t real_value, int16_t set_value, uint16_t deadzone, uint16_t precision)				
							fan_req = p_algorithm(real_temp,target_temp,dead_band,precision);
									//有加热
							if(devinfo_get_heater_level() == 0)
