...
int32_t can_send(uint8_t sa, uint8_t da, const uint8_t* data, uint16_t length, uint8_t prio, uint8_t raf)
{
    ...
    TxMessage.ExtId = 0;
    TxMessage.ExtId =
        prio << 27 | raf << 22 | sa << 14 |
        da << 6;  // set can user protocal priority, req/ack flag, destination addr and source addr segment
    TxMessage.StdId = 0;
    ...
    {
        res_count = res_len;
        TxMessage.ExtId |= CANID_EOSF_BITS;  // when programe run to this point, it must be the end of frame. thus add
                                             // the EOSF flag to the frame head
...
// can bus send data function
int32_t can_send_singleframe(uint8_t sa, uint8_t da, uint8_t type, const uint8_t* data, uint16_t length, uint8_t prio,
                             uint8_t raf)
{
    CanTxMsg TxMessage;
    ...
    if (TransmitMailbox == 0x04)  // if TransmitMailbox used up, abort transmission
    {
        ...
        //				rt_kprintf("TransmitMailbox used up, abort transmission  erro =
        //%d,canflag=%d\n",CAN_GetLSBTransmitErrorCounter(CAN1),CAN_GetFlagStatus(CAN1,CAN_FLAG_BOF)); when send buffer
        // > 3; physical offline
        return 0;
    }
    else
