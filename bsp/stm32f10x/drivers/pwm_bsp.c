...
PWM_AO1	<->	TIM8_CH1 PC_6
PWM_AO2	<->	TIM8_CH1 PC_7
PWM_AO3	<->	TIM8_CH1 PC_8
****************************************************/


//#define AO_CHAN_MAX_NUM		7	
#define AO_CHAN_MAX_NUM		5	
...
uint16_t pwm_set_ao(uint8_t channel, uint16_t ao_data)
{
		...
		else 
		{
				...
				{
						//÷ÿ”≥…‰
						u16AO=Sts_Remap(channel,Rep_AO,1);	
//						rt_kprintf("u16AO = %d,channel = %d,ao_data = %d\n",u16AO,channel,ao_data);						
						pwm_conf(u16AO,(100-ao_data));
						return 1;
				}
		}
}
